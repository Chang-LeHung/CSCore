# 30行自己写的并发工具类(Semaphore, CyclicBarrier, CountDownLatch)是什么体验？
...
>`semaphore.release`
>
>比如在上面的`acquire`和`release`之间的代码$\mathcal{R}$就是我们需要控制的代码，我们可以通过**信号量**控制在某一个时刻能有多少个线程执行代码$\mathcal{R}$。

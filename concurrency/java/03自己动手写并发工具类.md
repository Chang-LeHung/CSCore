...
在这一小节当中主要使用`ReentrantLock`实现上面我们提到的三个并发工具类，因此你首先需要了解`ReentrantLock`这个工具。`ReentrantLock`中有两个主要的函数`lock`和`unlock`，主要用于临界区的保护，在同一个时刻只能有一个线程进入被`lock`和`unlock`包围的代码块。

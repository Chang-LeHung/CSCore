...
在前面的内容当中我们已经仔细分析了完全背包问题的动态转移方程，我们发现在两层循环的内层循环当中，必须从`0`遍历到`V`，因为我们后面的数据依赖前面的数据，还有就是依赖上一行相同列的数据，因此我们可以使用一行数组去进行遍历即可，这个单行数组与多行数组的区别是多行数组每一行有每一行的状态，但是他有所有行的状态，单行数组只存在一行的状态，但是他会经历所有行的状态，也就是在外层`for`循环下单行数组的状态不断更新。
